...
public:
    ...
    void sort();
    //Function to sort the array for the binary search
    //Postcondition: the array is sorted O(n)

    void binarySearch(int number, int first, int last);
    //Funtion to search for a value in the array using binary search
    //Postcondition: return true if value is found
                  // else return false
...
template <class elemType>
void arrayListType<elemType>::sort() 
{
    if (listSize() == 0 || listSize() == 1)
    {
      return;
        ...
        int temp;
        length = listSize(); // Ensure length is updated
...
template <class elemType>
void arrayListType<elemType>::binarySearch(int number, int first, int last)
{
    // Calculate midpoint
    ...
    // Recursive cases: Search in the appropriate half
    if (list[mid] > number) {
        binarySearch(number, first, mid - 1);
    } else {
        binarySearch(number, mid + 1, last);
    }
}
