...
{
public:
    int FoldMethod(int HTSize, int studentID);
    // function to find hash using folding method
    // parameter passes hashTable size & student id
    ...
    int search(int& hashIndex, const elemType& rec, bool& found) const;
...
template<class elemType>
int hashT<elemType>::FoldMethod(int HTSize, int studentID)
{
    // for fold method, split keyX into halves and add together and store in HashResult to add to table
    int HashResult = 0;
    int keyX = studentID; // using keyX to make sure studentID doesnt change

    // make sure loop stops @ 0 to make sure all numbers in the key are checked
    while(keyX > 0)
        {
            // keyX %100 should remove the last two digits I believe? come back and try w/ 10 if errors
            int x = keyX % 100;
            HashResult += x; // add the digits to result and go again to fold the next half
            keyX /= 100; // remove the last two digits I think
        }

    // return the hash result
    return HashResult % HTSize; // %HTSize should return the number itself since it wount have a remainder? double check in post 
}
...
template <class elemType>
int hashT<elemType>::search(int& hashIndex, const elemType& rec,
    bool& found) const
{
    int inc = 1;
